    ...
    if (role === 'admin') {
      ...
      try {
        const res = await fetch('http://localhost:8766/login', {
          ...
          body: JSON.stringify({ email: username.trim(), contact: password.trim() })
        });
      ...
      try {
        const res = await fetch('http://localhost:8766/loginorg', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            orgemail: username.trim(),
            orgcontact: password.trim()
          })
        });

        if (res.ok) {
          const matchedOrg = await res.json();
          localStorage.setItem("deptid", matchedOrg.deptid);
          setResponse({ status: 'success', message: 'Organization login successful' });
          navigate('/organization-dashboard', { state: { organization: matchedOrg } });
        } else {
